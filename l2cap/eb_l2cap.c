...
#define EB_L2CAP_INVALID_IDX 0xFF
...
static uint8_t *get_rx_buf_by_conn_idx(int idx)
{
    return 0;
}
/**************************************************************
...
struct eb_l2cap {
    ...
    const struct eb_l2cap_send_cb *send_done_cbs;
...
struct eb_l2cap *eb_l2cap_init(struct eb_l2cap *l2cap, struct eb_l2cap_cfg *cfg)
{
    ...
    for (i = 0; i < cfg->max_connection; i++) {
        l2cap->conn[i].conn_idx = EB_L2CAP_INVALID_IDX;
        l2cap->conn[i].rx_buf = get_rx_buf_by_conn_idx(i);
    }
    l2cap->cb = cfg->cb;
...
{
    return sizeof(struct eb_l2cap) + max_connection * max_recv_buf_len;
}

static struct eb_l2cap_conn *get_l2cap_by_idx(struct eb_l2cap *l2cap, uint16_t conn_idx)
{
    EB_L2CAP_ERROR(l2cap, EB_L2CAP_DBG_ERR_PARAM);
    int i;
    for (i = 0; i < l2cap->max_connection; i++) {
        if (l2cap->conn[i].conn_idx == conn_idx) {
            return &l2cap->conn[i];
        }
    }
    return NULL;
...
static struct eb_l2cap_conn *set_l2cap_by_idx(struct eb_l2cap *l2cap, uint16_t conn_idx)
{
    EB_L2CAP_ERROR(l2cap, EB_L2CAP_DBG_ERR_PARAM);
    int i;
    if (get_l2cap_by_idx(l2cap, conn_idx)) {
        return NULL; // Already exist
    }
    for (i = 0; i < l2cap->max_connection; i++) {
        if (l2cap->conn[i].conn_idx == EB_L2CAP_INVALID_IDX) {
            l2cap->conn[i].conn_idx = conn_idx;
            return &l2cap->conn[i];
        }
    }
    return NULL;
...
{
    EB_L2CAP_ERROR(l2cap, EB_L2CAP_DBG_ERR_PARAM);
    EB_L2CAP_WARNING(!l2cap->send_done_cbs, EB_L2CAP_DBG_ERR_PARAM);
    l2cap->send_done_cbs = cb;
}
...
void eb_l2cap_received(struct eb_l2cap *l2cap, uint16_t hdl_flags, uint16_t datalen, uint8_t *payload)
{
    ...
    if (conn) {
        ...
        if (conn->rx_remain_len == 0) { // Data reassembled, callback then
            ...
            p += sizeof(uint16_t); // Skip CID
            l2cap->proc_cb(conn_idx, cid, p, l2cap_len, l2cap->usr_data);
        }
    } else {
...
void eb_l2cap_connected(struct eb_l2cap *l2cap, uint16_t conn_idx, uint8_t role,
                        uint8_t *peer_addr, uint8_t peer_addr_type, uint8_t *local_addr, uint8_t local_addr_type)
{
    EB_L2CAP_ERROR(l2cap, EB_L2CAP_DBG_ERR_PARAM);
    struct eb_l2cap_conn *conn = set_l2cap_by_idx(l2cap, conn_idx);
    EB_L2CAP_WARNING(conn, EB_L2CAP_DBG_ERR_NO_CONN);
    if (conn) {
        conn->rx_remain_len = 0;
        conn->pending_acl_packets = 0;
        if (l2cap->connected_cb) {
            l2cap->connected_cb(conn_idx, role, peer_addr, peer_addr_type, local_addr, local_addr_type, l2cap->usr_data);
        }
    }
}

void eb_l2cap_disconnected(struct eb_l2cap *l2cap, uint16_t conn_idx)
{
    EB_L2CAP_ERROR(l2cap, EB_L2CAP_DBG_ERR_PARAM);
    struct eb_l2cap_conn *conn = get_l2cap_by_idx(l2cap, conn_idx);
    EB_L2CAP_WARNING(conn, EB_L2CAP_DBG_ERR_NO_CONN);
    if (conn) {
        ...
        conn->conn_idx = EB_L2CAP_INVALID_IDX;
        if (l2cap->disconnected_cb) {
            l2cap->disconnected_cb(conn_idx, l2cap->usr_data);
        }
    }
}
...
void eb_l2cap_acl_cfg(struct eb_l2cap *l2cap, uint16_t pkg_size, int pkg_num)
{
    int i;
    for (i = 0; i < l2cap->max_connection; i++) {
        EB_L2CAP_WARNING(l2cap->conn[i].conn_idx == EB_L2CAP_INVALID_IDX);
    }
...
void eb_l2cap_packets_completed(struct eb_l2cap *l2cap, uint16_t conn_idx, int pkg_num)
{
    EB_L2CAP_ERROR(l2cap, EB_L2CAP_DBG_ERR_PARAM);
    struct eb_l2cap_conn *conn = get_l2cap_by_idx(l2cap, conn_idx);
    if (conn) {
    ...
    while (l2cap->num_le_acl_data_packets) {
        ...
        if (conn) {
            ...
            if (p) {
                ...
                uint16_t conn_idx = hdl_flags & EB_L2CAP_HANDLE_MASK;
                EB_L2CAP_WARNING(conn->conn_idx == conn_idx);
                *--p = 0x02; // Add ACL Mark
